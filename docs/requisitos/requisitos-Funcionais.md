# Requisitos Funcionais

## RF01 — Catálogo de motores

O sistema deverá permitir que visitantes visualizem os modelos de motores comercializados ou apresentados pela FeioDiesel, com foco em motores Scania.

## RF02 — Informações técnicas do motor

O sistema deverá apresentar informações técnicas referentes a cada modelo de motor, incluindo suas principais características e especificações.

## RF03 — Aplicações do motor

O sistema deverá apresentar os modelos de caminhões e aplicações relacionadas a cada motor cadastrado.

## RF04 — Vantagens do motor

O sistema deverá apresentar as principais vantagens e características de cada modelo de motor, permitindo que o cliente conheça melhor suas aplicações.

## RF05 — Conteúdo educacional dos componentes

O sistema deverá disponibilizar um conteúdo educacional relacionado aos componentes de cada modelo de motor.

## RF06 — Visualização interativa dos componentes

O sistema deverá permitir que o visitante explore os componentes do motor de maneira simples e interativa, visualizando informações sobre o nome, função e importância de cada componente.

## RF07 — Imagens dos componentes

O sistema deverá permitir a associação de imagens específicas aos componentes apresentados em cada modelo de motor.

## RF08 — Solicitação de agendamento

O sistema deverá permitir que o cliente solicite o agendamento para montagem de um motor.

## RF09 — Cadastro do cliente

O sistema deverá permitir o registro das informações necessárias do cliente para realização de uma solicitação de agendamento.

## RF10 — Dados do cliente

O sistema deverá armazenar as informações do cliente necessárias para contato, incluindo nome, CPF, e-mail e telefone.

## RF11 — Caminhão opcional

O sistema deverá permitir que o cliente realize uma solicitação de montagem mesmo que não possua um caminhão.

## RF12 — Informações do caminhão

Quando o cliente possuir um caminhão, o sistema deverá permitir o registro das informações necessárias para auxiliar na identificação e análise de compatibilidade do motor.

## RF13 — Identificação do veículo

O sistema deverá permitir o registro do número de chassi/VIN do caminhão.

## RF14 — Informações técnicas do veículo

O sistema deverá permitir o registro de informações técnicas do caminhão, incluindo modelo ou nomenclatura, ano, geração, configuração do bloco e cilindros, sistema de injeção e sistema de emissões.

## RF15 — Motor anterior

Quando necessário para análise da solicitação, o sistema deverá permitir o registro do número de série do motor anteriormente utilizado no caminhão.

## RF16 — Recomendação de motor

O sistema deverá utilizar as informações disponíveis do caminhão para auxiliar na identificação e recomendação de um modelo de motor compatível.

## RF17 — Análise da solicitação

O sistema deverá permitir que a FeioDiesel analise as informações fornecidas pelo cliente antes da confirmação da montagem.

## RF18 — Confirmação do agendamento

O sistema deverá permitir que a FeioDiesel confirme a solicitação após a análise das informações e o contato com o cliente.

## RF19 — Prazo de montagem e entrega

O sistema deverá permitir o registro das informações relacionadas ao prazo estimado de montagem e deslocamento do motor.

## RF20 — Acompanhamento do processo

O sistema deverá permitir o acompanhamento do estado da solicitação, desde o recebimento até a conclusão do processo de montagem.

## RF21 — Área administrativa

O sistema deverá possuir uma área administrativa restrita para gerenciamento das informações do sistema.

## RF22 — Autenticação administrativa

O sistema deverá exigir autenticação para acesso à área administrativa.

## RF23 — Gerenciamento de motores

O administrador deverá poder cadastrar, visualizar, alterar e remover informações dos modelos de motores disponibilizados no sistema.

## RF24 — Gerenciamento do conteúdo educacional

O administrador deverá poder cadastrar, alterar e organizar os componentes e conteúdos educacionais relacionados aos motores.

## RF25 — Gerenciamento de agendamentos

O administrador deverá poder visualizar e gerenciar as solicitações de agendamento realizadas pelos clientes.

## RF26 — Gerenciamento de clientes

O administrador deverá poder visualizar e administrar os cadastros de clientes necessários ao funcionamento do sistema.

## RF27 — Gerenciamento de informações

O administrador deverá poder modificar informações apresentadas no site sem necessidade de alterar diretamente o código-fonte da aplicação.
